...
describe('Either monad tests', () => {
    ...
    it('should test either map with non null right', async () => {
        const maybeNonNull = _fish
            .either(null, 0)
            .map((val) => val + 100)
            .map((val) => val * 2)
    ...
    it('should test either map with null right', async () => {
        const errorMessage = 'some left error';
        const maybeNonNull = _fish
            .either(errorMessage, 0)
            .map((val) => val + 100)
            .map(() => undefined)
            .map((val) => val + 100)
            .fold((val) => `error happened with ${val}`, (val) => val);
    ...
    it('should test either map with folded either inside', async () => {
        const errorMessage = 'some left error';
        const eitherFnWithFold = (val: number) =>
            _fish
                .either(errorMessage, val)
                .map((val) => val)
                .fold((val) => val, (val) => val * 2);
        const maybeNonNull = _fish
            .either(null, 0)
            .map((val) => val + 100)
            .map((val) => val * 2)
    ...
    it('should test either flatmap', async () => {
    ...
    it('should test either flatmap with empty value inside', async () => {
        const insertedEitherError = 'insert either error';
        const eitherFn = (val: number) =>
            _fish
                .either(insertedEitherError, val)
                .map(() => undefined)
                .map((val) => val + 10);
        const maybeNonNull = _fish
            .either(null, 0)
    ...
    it('should test either map with empty value before conditional flatMap with custom error result', async () => {
        const errorMessage = 'some left error';
    ...
    it('should test either flatMap with new either result inside and with empty map after', async () => {
        const errorMessage = 'some left error';
        const customErrorMessage = 'custom error';
    ...
    it('should test either flatMap with new either result inside', async () => {
        const errorMessage = 'some left error';
        const customErrorMessage = 'custom error';
        const someNewEitherErrorMessage = 'some new either error value';
        const result = 49;
